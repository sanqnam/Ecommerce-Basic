...
const Popup = () => {
    ...
    return (
        <dialog ref={dialogRef} className="p-6 w-1/2 m-auto pt-10 overflow-hidden rounded-md shadow">
            <button
            ...
            <div className="flex gap-4">
                ...
                <div className="w-1/2">
                    <h2 className="text-xl font-bold italic">{product.name}</h2>
                    <p className="mt-2 font-semibold text-gray-500 text-xl italic">{formatCurrency(product.price)}</p>
                    <p className="mt-2 text-sm text-gray-600 line-clamp-7 overflow-hidden text-ellipsis mb-3">{product.short_desc}</p>

                    <NavLink  to={`/shop/detail/${product._id.$oid}`} onClick={()=>dispatch(HIDE_POPUP())} className="bg-gray-800 text-gray-50 italic py-2 px-4">
